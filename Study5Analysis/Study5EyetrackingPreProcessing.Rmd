...
#write.csv(ControlETdata, "ControlETdata.csv")

VisualETdata <- ETdata %>% 
  subset(Condition == "Visual")

#write.csv(VisualETdata, "VisualETdata.csv")


AvatarETdata <- ETdata %>% 
  subset(Condition == "Avatar")

write.csv(AvatarETdata, "AvatarETdata.csv")


NDRTETdata <- ETdata %>% 
  subset(Condition == "NDRT")

#write.csv(NDRTETdata, "NDRTETdata.csv")
...
```{r}
test <- ControlETdata
 startTime = test$eyeDataRelativeTimestamp[1]
  test$TimeStamp <- (test$eyeDataRelativeTimestamp - startTime) / 1000
...
for(dataset in listControldata) {
  ...
  data$TimeStamp <- (data$eyeDataRelativeTimestamp - startTime) / 1000
...
#   select(-X)
#AOIAvatarETData$TimeStamp <- AOIAvatarETData$TimeStamp*10
...
#AOINDRTETDataDone <- AOINDRTETData
#listNDRTdata<- listNDRTdata[-c(1:7)]
...
# AOINDRTETDataComplete <- rbind(AOINDRTETDataDone, AOINDRTETData)
#  
#  write.csv(AOINDRTETDataComplete, "AOINDRTETData.csv")
...
#ValidAOIVisualETData <- subset( ValidAOIVisualETData, select = -X )

FinalValidAOIETData <- rbind(ValidAOIControlETData, ValidAOIVisualETData, ValidAOIAvatarETData, ValidAOINDRTETData)
...
NewAvatar <- read.csv("C:/Users/thoma/OneDrive - University of Glasgow/Documents/R_Scripts/Study5Analysis/FixValidAOIAvatarETData.csv", header = T)
...
ValidAOINDRTETData <- FinalValidAOIETData %>% 
  subset(Condition == "NDRT")
```
...
NDRTPlot <- ggplot(data=ValidAOINDRTETData, aes(x=fixation_x, y=fixation_y)) +
  # geom_point()+
  # geom_bin2d()+
  stat_bin_2d(bins = binwidth)+
  #   xlim(-0.4, 0.4)+
...
FullFixationPlot <- plot_grid(ControlPlot, VisualPlot, AvatarPlot, NDRTPlot,ncol = 2)
FullFixationPlot
