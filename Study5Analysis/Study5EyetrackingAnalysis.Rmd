...
TotalNoFPlot <-ggplot(data = PptSummaryAOI, aes(x = Condition, y = NoF, fill = Condition))+
 ...
 # ylim(0,100)+
  labs(title = "Mean Number of Overall Fixations", y = "Mean Fixation Count", x = "NDRT Presentation Condition")+ # ylim(0,100)+
scale_fill_manual(values = S5_palette) +
  theme_classic()+
...
TotalHazardNoFPlot <- ggplot(data = CorrHazardFixCountPpt, aes(x = Condition, y = NoF, fill = Condition))+
  ...
  labs(title = "Mean Number of Fixations \n during the Hazard Window", y = "Mean Fixation Count", x = "NDRT Presentation Condition")+scale_fill_manual(values = S5_palette) +
  ylim(0,20)+
  theme_classic()+
  theme(legend.position="none",
...
TTFF$Condition <- relevel(TTFF$Condition, ref = "NDRT")
...
TTFFPlot
ggsave(filename = "S5TTFFPlot.jpg",
       plot = TTFFPlot)
