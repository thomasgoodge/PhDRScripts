...
ggplot(data = WHNSummary, aes(x = Block, y = WHN, fill = Block))+
  ...
  labs(title = "Average WHN Score", ylim(0,8))+
  scale_fill_hue(l = 40)
...
#######CONFIDENCE SCORES##############
```{r}
ConfBaseline <- as.numeric(WHNSummary[1,5])
...
ggplot(data = WHNSummary, aes(x = Block, y = meanConf, fill = Block))+
  ...
  labs(title = "Average Confidence Score", xlim(0,1.0))+
  scale_fill_hue(l = 40)
```
...
```{r}
WHNNullModel <- lmer(data = WHNDataset, formula = trialResp.corr ~  (1|participant))
...
```{r}
WHNModel1 = lmer(data = WHNDataset, formula = trialResp.corr ~ Block + (1|participant))
...
```{r}
ConfNullModel <- lmer(data = WHNDataset, formula = conf_slider.response ~  (1|participant))
...
```{r}
ConfModel1 = lmer(data = WHNDataset, formula = conf_slider.response ~ Block + (1|participant))
...
```{r}
MISCfModel1 = lmer(data = WHNDataset, formula = MiserScale.response ~ Block + (1|participant))
