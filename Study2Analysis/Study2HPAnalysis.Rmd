...
```{r}
tg <- lm(RTStandard ~ Condition * Block, data = RTHazardData)

summary(tg)

t2g <- lm(RTStandard ~ Condition + Block, data = RTHazardData)
...
```{r}
mm <- lme4::lmList(RTStandard~Block|Condition,RTHazardData)
coef(mm)
...
```{r}
mm2 <- lme4::lmList(RTStandard~Condition|Block,RTHazardData, pool =  T)
coef(mm2)
...
```{r}
NullModel <- lmer(data = RTHazardData, formula = RTStandard ~  (1|participant))
...
```{r}
Model1 = lmer(data = RTHazardData, formula = RTStandard ~ Block + Condition + Block * Condition + (1|participant))
