...
dataFolder = "C:/Users/thoma/OneDrive - University of Glasgow/Desktop/Study 6/Study6Data/ARTasks"
...
scoreDatasetRaw[c('Ppt','Block')] <- str_split_fixed(scoreDatasetRaw$`"ID"`, 'S6', 4)
...
write.csv(scoreDataset, "C:/Users/thoma/OneDrive - University of Glasgow/Desktop/Study 6/ComparisonData/S6NDRT.csv", row.names=FALSE)
...
data_long <- data %>%
  ...
  pivot_longer(cols = `1`:`4`, names_to = "Number", values_to = "Frequency") %>% 
    mutate(Condition = factor(Condition, levels = c( "Visual", "Avatar","Social", "Control")))

# Plotting
S6RankingPlot <- ggplot(data_long, aes(x = Condition, y = Frequency, fill = Number)) +
  ...
  scale_fill_manual(values = S6_palette) +  # Custom colors
  coord_flip() +  # Flip the plot to horizontal
  theme_minimal()

S6RankingPlot

ggsave(filename = "S6RankingPlot.png",
       plot = S6RankingPlot,
       device = png)
