...
binwidth <- 100  # Adjust the binwidth as needed
...
ValidVisual$gazePointAOI_name[ValidVisual$gazePointAOI_name == 'VisualCueAOI'] <- 'VisualTargetAOI'
...
# HazardARFixLMER <- lmer(n ~ Condition + (1|gazePointAOI_name), data = HazardAOIARTargets)
...
TTFFWallModelLMER <- lmer(data = TTFFWallSingle, formula = eventDuration ~ Condition + (1|ID))

TTFFWallModelLMER2 <- lmer(data = TTFFWallSingle, formula = eventDuration ~ Condition + (1|ID) + (1|VideoClip))

TTFFWallModelLMER3 <- lmer(data = TTFFWallSingle, formula = eventDuration ~ Condition + (1|ID) + (1|VideoClip) + (1|gazePointAOI_name))

TTFFWallModelLMER4 <- lmer(data = TTFFWallSingle, formula = eventDuration ~ Condition + (1|ID) + (1|VideoClip)+ (1|gazePointAOI_name) + (1|Hazard.Location) )

summary(TTFFWallModelLMER3)
ranef(TTFFWallModelLMER3)
fixef(TTFFWallModelLMER3)

anova(TTFFWallModelLMER,TTFFWallModelLMER2,TTFFWallModelLMER3,TTFFWallModelLMER4)

```


```{r}
summary(TTFFWallModelLMER)
...
TTFFWallSingle$Condition <- factor(TTFFWallSingle$Condition, levels = c("Control", "Visual",  "Avatar","Social"))
...
TTFFPlot <- ggplot(TTFFWallSingle, aes(x = Condition, y = FirstFixationTime, fill = Condition))+
  ...
  ylim(0,4)+
  labs(title = "Time to First Fixation (ms) \n \n", y = "Mean Time to First Fixation", x = "NDRT Presentation Condition")+
  scale_fill_manual(values = S6_palette) +
  theme_classic()
