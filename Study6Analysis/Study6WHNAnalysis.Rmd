...
#power
```{r}
pwr.t.test(n = 24, sig.level = 0.05, d = 0.814646, type = "two.sample")
...
#WHN Col Plot
```{r}
WHNDataset$Block <- relevel(WHNDataset$Block, ref = "Control - 2")
...
AttPlot2 <- ggplot(data = WHNDataset, aes(x = Block, y = Attention_slider.response, fill = Block))+
  geom_violin(alpha = 0.8)+geom_boxplot(alpha = 0.2, width = 0.1)+
...
WHNDataset$Block <- relevel(WHNDataset$Block, ref = "Social Cue")
...
WHNDataset$Block <- relevel(WHNDataset$Block, ref = "Avatar Cue")
...
summary(ConfModel)
 ...
 # summary(ConfModel1)
 # 
 ConfModel2 = glmer(data = WHNDataset, formula = conf_slider.response ~ Block + (1|participant) + (1 + image_file|participant), family = binomial, glmerControl(optimizer = "bobyqa"))
 # 
