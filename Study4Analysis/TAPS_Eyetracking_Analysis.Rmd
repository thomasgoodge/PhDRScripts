...
TTFFWallModelBasic <- lm(data = TTFFWallSingle, formula = FirstFixationTime ~ Condition.x)

#DurModelBasic <- lmer(data = PptSummaryAOI, formula = meanDuration ~ Condition + (1|Ppt))
TTFFWallModelBasic <- lmer(data = TTFFWallSingle, formula = FirstFixationTime ~ Condition.x + (1|ID))

TTFFWallModel2 <- lmer(data = TTFFWallSingle, formula = FirstFixationTime ~ Condition.x + (1|ID) + (1|gazePointAOI_name))

summary(TTFFWallModel2)
...
```{r}
ggplot(TTFFWallSingle, aes(x = Condition.x, y = FirstFixationTime, fill = Condition.x))+
  geom_violin()+ 
