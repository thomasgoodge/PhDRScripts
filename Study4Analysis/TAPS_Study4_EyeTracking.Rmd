...
CorrHazardCountData <- FinalValidHazardAOIETDataRMOut$Sort 
...
FixCountSummary <- CorrHazardCountData %>% 
...
FixCountSummary$Condition <- factor(FixCountSummary$Condition, levels = c( "HUD", "HDD", "Cue","Control"))

CountGLM <- glm(data = FixCountSummary, n ~ Condition, family = poisson)
summary(CountGLM)

CountGLMScreen <- glmer(data = FixCountSummaryScreen, n ~ Condition + (1|pa), family = poisson, )
summary(CountGLMScreen)

#FixCountSummaryScreen$Condition <- relevel(FixCountSummaryAR$Condition, ref = "HUD")

#FixCountSummaryAR$Condition <- as.factor(FixCountSummary$Condition)
#FixCountSummaryAR$Condition <- relevel(FixCountSummaryAR$Condition, ref = "HUD")

CountGLMAR <- glm(data = FixCountSummaryAR, n ~ Condition, family = poisson)
summary(CountGLMAR)

```

```{r}
ggplot(data = FixCountSummary, aes(x = Condition, y = n, fill = Condition))+
  geom_col()+ 
...
```
```{r}
ggplot(data = CorrHazardCountData, aes(x = Condition, y = CorrAOI, fill = Condition))+
  geom_col()+ 
...
ggplot(data = CorrHazardCountData, aes(x = Condition, y = eventDuration, fill = Condition))+
  geom_violin()+ 
  geom_boxplot(width = 0.2) +
  ylab("Duration of  Correct Fixations")+
 # scale_fill_brewer(palette = "BrBG")+
  theme_classic()
...
```{r}
NullCorrModel <- glmer(CorrAOI ~ (1|ID), family = binomial, data = CorrHazardCountData)
summary(NullCorrModel)

BasicCorrModel <- glmer(CorrAOI ~ Condition + (1|ID), family = binomial(link = "logit"), data = CorrHazardCountData)
summary(BasicCorrModel)

CorrModel1 <- glmer(CorrAOI ~ Condition + (1|ID) + (1|VideoClip), family = binomial(link = "logit"), data = CorrHazardCountData)
summary(CorrModel1)

# CorrModel2 <- glmer(CorrAOI ~ Condition + (1|ID) + (1|VideoClip) + (1|TimeStamp), family = binomial(link = "logit"), data = CorrHazardCountData)
# summary(CorrModel2) # no significant effect of Time

CorrModel2 <- glmer(CorrAOI ~ Condition + (1|ID) +(1|VideoClip)+  (1|AOILocationOffset), family = binomial(link = "logit"), data = CorrHazardCountData)
summary(CorrModel2)

#FinalValidHazardAOIETDataRMOut$Condition <- relevel(FinalValidHazardAOIETDataRMOut$Condition, ref = "HUD")

# CorrModel3 <- glmer(CorrAOI ~ Condition + (1|ID) +(1|VideoClip) + (1|AOILocationOffset) + (1 + ID|VideoClip), family = binomial(link = "logit"), data = FinalValidHazardAOIETDataRMOut) # too big and complicated

# CorrModel3 <- glmer(CorrAOI ~ Condition + (1|ID) +(1|VideoClip)+  (1|AOILocationOffset), family = binomial(link = "logit"), data = FinalValidHazardAOIETDataRMOut)
...
CorrHazardCountData$Condition <- relevel(CorrHazardCountData$Condition, ref = "HUD")
 ...
 # CorrModel3 <- glmer(CorrAOI ~ Condition + (1|ID) +(1|VideoClip) + (1|AOILocationOffset) + (1 + ID|VideoClip), family = binomial(link = "logit"), data = CorrHazardCountData) # too big and complicated
 # 
 # CorrModel3 <- glmer(CorrAOI ~ Condition + (1|ID) +(1|VideoClip)+  (1|AOILocationOffset), family = binomial(link = "logit"), data = CorrHazardCountData)
...
ggplot(data = FinalValidHazardAOIETDataRMOut, aes(x = Condition, y = CorrAOI, fill = Condition))+
  geom_col()+ 
  ylab("Number of 'Correct' Fixations")+
  scale_fill_brewer(palette = "BrBG")+
  theme_classic()
...
#CountHazardFixationDuration
```{r}
CorrHazardCountData$Condition <- as.factor(CorrHazardCountData$Condition)
CorrHazardCountData$Condition <- relevel(CorrHazardCountData$Condition, ref = "Cue")
...
```{r}
ggplot(CorrHazardCountData, aes(x = Condition, y = eventDuration, fill = Condition))+
  geom_violin()+ 
  geom_boxplot(alpha = 0, width = 0.18)+
  ylab("Fixation Duration in Hazard Window")+
  theme_classic()
